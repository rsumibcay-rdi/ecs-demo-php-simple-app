...
return '<a class="excerpt-read-more" href="'. get_permalink($post->ID) . '" title="'. __('Read', 'cybergrx') . get_the_title($post->ID).'">'. __('... ', 'cybergrx') .'</a>';
}*/
...
function joints_get_the_author_posts_link() {
	...
	$link = sprintf(
		...
		esc_attr( sprintf( __( 'Posts by %s', 'cybergrx' ), get_the_author() ) ), // No further l10n needed, core will take care of this one
		get_the_author()
	);
	return $link;
}